...
try:
    ...
    block_dropdown_locator = (By.XPATH, "//label[contains(text(), 'Block')]/following-sibling::ng-multiselect-dropdown")
    block_select_all_locator = (By.XPATH, "//label[contains(text(), 'Block')]/following-sibling::ng-multiselect-dropdown//div[text()='Select All']")
    ...
    for state in state_names:
        ...
        for attempt in range(max_retries):
            try:
                ...
                # --- 4. Select "BLOCK" from Top view ---
                top_view_element = wait.until(EC.presence_of_element_located(top_view_locator))
                # --- FIX: Wait for the 'BLOCK' option to be loaded before trying to select it ---
                wait.until(EC.presence_of_element_located((By.XPATH, "//label[contains(text(), 'Top view')]/following-sibling::select/option[text()='BLOCK']")))
                select_top_view = Select(top_view_element)
                select_top_view.select_by_visible_text("BLOCK")
                print("  -> Selected 'BLOCK' from Top view.")
                wait.until(EC.invisibility_of_element_located(overlay_locator))
                ...
                driver.execute_script("arguments[0].click();", district_box) # Open dropdown
                select_all_districts = wait.until(EC.presence_of_element_located(district_select_all_locator))
                driver.execute_script("arguments[0].click();", select_all_districts) # Click select all
                driver.execute_script("arguments[0].click();", district_box) # Close dropdown
                ...
                # --- 6. Select all Blocks ---
                block_box = wait.until(EC.presence_of_element_located(block_dropdown_locator))
                driver.execute_script("arguments[0].click();", block_box) # Open dropdown
                select_all_blocks = wait.until(EC.presence_of_element_located(block_select_all_locator))
                driver.execute_script("arguments[0].click();", select_all_blocks) # Click select all
                driver.execute_script("arguments[0].click();", block_box) # Close dropdown
                print("  -> Selected 'Select All' for blocks.")
                wait.until(EC.invisibility_of_element_located(overlay_locator))
