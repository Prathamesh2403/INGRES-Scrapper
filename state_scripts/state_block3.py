    ...
    for state in state_names:
        ...
        for attempt in range(max_retries):
            try:
                ...
                is_island_case = False
                sub_division_label = None
                
                sub_division_types = ["VALLEY", "TEHSIL", "TALUK", "MANDAL", "BLOCK"]
                
                for sd_type in sub_division_types:
                    if sd_type in top_view_options:
                        sub_division_label = sd_type.title()
                        break

                if "ISLAND" in top_view_options:
                    select_top_view.select_by_visible_text("ISLAND")
                    is_island_case = True
                    print("  -> Selected 'ISLAND' from Top view.")
                elif sub_division_label:
                    select_top_view.select_by_visible_text(sub_division_label.upper())
                    print(f"  -> Selected '{sub_division_label.upper()}' from Top view.")
                elif "DISTRICT" in top_view_options:
                    select_top_view.select_by_visible_text("DISTRICT")
                    print("  -> Selected 'DISTRICT' from Top view.")
                else:
                    raise Exception("Could not find a valid Top view option.")
                ...
                try:
                    ...
                    print("  -> No district selection box found, skipping.")

                # --- 5b. Select all Regions (if applicable) ---
                try:
                    region_dropdown_locator = (By.XPATH, f"//label[contains(text(), 'Region')]/following-sibling::ng-multiselect-dropdown")
                    region_select_all_locator = (By.XPATH, f"//label[contains(text(), 'Region')]/following-sibling::ng-multiselect-dropdown//div[text()='Select All']")
                    
                    region_box = wait.until(EC.presence_of_element_located(region_dropdown_locator))
                    driver.execute_script("arguments[0].click();", region_box)
                    select_all_regions = wait.until(EC.presence_of_element_located(region_select_all_locator))
                    driver.execute_script("arguments[0].click();", select_all_regions)
                    driver.execute_script("arguments[0].click();", region_box)
                    print(f"  -> Selected 'Select All' for Regions.")
                    wait.until(EC.invisibility_of_element_located(overlay_locator))
                    time.sleep(1)
                except:
                    pass 

                # --- 6. Select all Sub-divisions ---
                if sub_division_label:
                    try:
                        ...
                        print(f"  -> No {sub_division_label} selection box found, skipping.")

                # --- 6b. Select all Islands (if applicable) ---
                if is_island_case:
                    try:
                        island_dropdown_locator = (By.XPATH, f"//label[contains(text(), 'Island')]/following-sibling::ng-multiselect-dropdown")
                        island_select_all_locator = (By.XPATH, f"//label[contains(text(), 'Island')]/following-sibling::ng-multiselect-dropdown//div[text()='Select All']")
                        
                        island_box = wait.until(EC.presence_of_element_located(island_dropdown_locator))
                        driver.execute_script("arguments[0].click();", island_box)
                        select_all_islands = wait.until(EC.presence_of_element_located(island_select_all_locator))
                        driver.execute_script("arguments[0].click();", select_all_islands)
                        driver.execute_script("arguments[0].click();", island_box)
                        print(f"  -> Selected 'Select All' for Islands.")
                        wait.until(EC.invisibility_of_element_located(overlay_locator))
                        time.sleep(1)
                    except:
                        print("  -> No Island selection box found, skipping.")
