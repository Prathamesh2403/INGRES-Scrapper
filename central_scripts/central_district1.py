#this script works but selects all districts one by one
...
try:
    ...
    # --- NEW: Locator for individual districts ---
    district_options_locator = (By.XPATH, "//label[contains(text(), 'District')]/following-sibling::ng-multiselect-dropdown//div[@class='dropdown-list']/ul[2]/li/div")
    ...
    for state in state_names:
        ...
        for attempt in range(max_retries):
            try:
                ...
                # --- NEW LOGIC: Select all districts one-by-one ---
                print("    -> Waiting for district list to appear...")
                wait.until(EC.visibility_of_element_located(district_options_locator))
                district_elements = driver.find_elements(*district_options_locator)
                print(f"    -> Found {len(district_elements)} districts. Selecting them now.")

                for district_element in district_elements:
                    district_element.click()
                    time.sleep(0.1)
                print("    -> Finished selecting all districts.")
