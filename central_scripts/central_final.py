...
try:
    ...
    js_script = f"arguments[0].value = '{target_year}'; arguments[0].dispatchEvent(new Event('change'));"
    driver.execute_script(js_script, year_select_element)
    
    # --- 2. Select a Single State ---
    ...
    print("  -> Opening State dropdown using ActionChains...")
    ...
    if state_names:
        target_state = state_names[0]
        print(f"    - Selecting first state: {target_state}")
        state_to_click = wait.until(EC.element_to_be_clickable((By.XPATH, f"//li/div[text()='{target_state}']")))
    ...
    print("  -> Waiting for final data refresh to complete...")
    wait.until(EC.invisibility_of_element_located(overlay_locator))
    print("  -> Data refresh complete.")

    # --- 4. Generate Report ---
    print("\n- Clicking GENERATE REPORT button using JavaScript...")
    generate_button = wait.until(EC.element_to_be_clickable(generate_report_locator))
    driver.execute_script("arguments[0].click();", generate_button)
    print("✅ Report generation started. Waiting for download to complete...")
