...
Based on a review of the codebase, the following improvements are recommended:

**1. Consolidate into a single configurable entry point**
Create a `config.py` or `.env` file for `target_year`, `report_type`, and `download_dir` so
users do not need to edit source code directly.

**2. Add CLI arguments**

```bash
python scrape.py --year 2022-2023 --type central
python scrape.py --year 2020-2021 --type state-block
```

**3. Add resume capability**
Log completed states to a `progress.log` file so interrupted runs can skip already-downloaded states
...
**5. Use python-dotenv for the portal URL**
The `python-dotenv` package is already a dependency but unused. Move the hardcoded INGRES URL to
a `.env` file so it can be updated without touching source code.

**6. Add headless mode option**
Allow Chrome to run without a visible window for server or CI environments:
